...
| `POST` | `/oauth/authorize`, `/oauth/token` | Dev/mock OAuth for clients (disable with `CAST_HUB_MOCK_OAUTH=false`) |

### Authentication modes

| Mode | Hub settings | Client behavior |
|------|--------------|-----------------|
| **Local dev (default)** | `CAST_HUB_MOCK_OAUTH=true`, `CAST_HUB_REQUIRE_AUTH=false` | Clients `POST` hub `/oauth/*`; subscribe works without enforced Bearer checks |
| **Enforced mock tokens** | `CAST_HUB_REQUIRE_AUTH=true` | Subscribe/publish/request require Bearer JWT from mock `/oauth/token` |
| **Keycloak RS256** | `CAST_HUB_OIDC_ISSUER` + optional `CAST_HUB_REQUIRE_AUTH=true` | Hub validates Keycloak access tokens via JWKS; deploy IdP from [`cast-keycloak`](https://github.com/cast-interface/cast-keycloak) |

Bearer validation lives in [`Lib/oauth_tokens.py`](Lib/oauth_tokens.py) (HS256 mock + RS256/JWKS).
