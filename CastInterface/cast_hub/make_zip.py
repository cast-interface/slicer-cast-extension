...
packages everything under cast_hub/:
...
def find_repo_root(script_dir: Path, key: str) -> Path | None:
    """Locate client repos from cast_api upward (workspace roots).

    Prefer ``ProjectWeek45/<repo>`` for VolView, vtk-js, and Viewers.
    Slim is a workspace sibling at ``<workspace>/slim`` (not under ProjectWeek45).
    """
