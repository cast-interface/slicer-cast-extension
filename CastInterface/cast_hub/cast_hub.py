...
from oauth_tokens import (
    CAST_HUB_MOCK_OAUTH,
    CAST_HUB_OIDC_ISSUER,
    CAST_HUB_REQUIRE_AUTH,
    JWT_AUDIENCE,
    JWT_DEFAULT_SECRET,
    JWT_ISSUER,
...
def _register_spa_client(app, mount_path: str, client_folder: str) -> bool:
    """Serve a built SPA at /{mount_path}/ from cast_hub/{client_folder}/."""
        ...
        if full_path == "favicon.ico":
            ...
            if mount_path == "worklist-client":
                fallback = os.path.join(
                    resources_dir, "images", "cast-radio-favicon.svg"
                )
...
def _enforce_bearer_auth(request: Request) -> Dict[str, Any]:
    """When CAST_HUB_REQUIRE_AUTH is set, require a valid Bearer JWT."""
    try:
...
def _serve_html_page(filename: str):
    """Serve an HTML page from cast_hub/Resources/, fallback to static mount."""
...
async def get_hub_samples(request: Request):
    """Worklist catalog for hub-hosted sample studies under cast_hub/samples/."""
...
def _read_cast_extension_version() -> str:
    """Extension/hub version for admin About (cast_hub/VERSION or parent CastInterface/VERSION)."""
    for candidate in (
...
async def get_admin_about():
    ...
    return {
        ...
        "homepage": "https://github.com/cast-interface/slicer-cast-extension/",
    }
...
def main():
    ...
    if CAST_HUB_REQUIRE_AUTH:
        print("  Bearer auth: required on /api/hub and /api/hub/request")
    if CAST_HUB_OIDC_ISSUER:
        print(f"  OIDC issuer: {CAST_HUB_OIDC_ISSUER}")
    print("")
