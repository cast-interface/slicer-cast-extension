...
from .repo_paths import cast_hub_dir, cast_hub_script, repo_root
...
class CastHubWidget:
    """Hub section UI: start/stop local cast_hub subprocess."""
    ...
    def _on_start(self) -> None:
        if self._is_running():
            return
        cast_hub_path = str(cast_hub_dir())
        script = self._hub_script_path()
        if not os.path.isfile(script):
            self._set_status(_("cast_hub.py not found"), _STATUS_TEXT_STYLE_ERROR)
            return
        ...
        proc.setWorkingDirectory(cast_hub_path)
