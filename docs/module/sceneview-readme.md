...
## Browser image display (VolView)

VolView answers `sceneview-request` on the bind WebSocket when subscribed as image display (`ID` actor). The `sceneview-response` payload is JSON on `event.context.data` (collated HTTP `responses[].data`):

| Field | Description |
|-------|-------------|
| `source` | `"sceneview"` |
| `product` | Responder product name (e.g. `VOLVIEW`) |
| `window` | `screenX`, `screenY`, `outerWidth`, `outerHeight`, `innerWidth`, `innerHeight` |
| `display.layoutName` | Named layout (e.g. `3D Primary`) |
| `display.layout` | Nested layout tree (`direction`, `items`) |
| `display.activeViewId` | Focused view id |
| `display.maximized` | Whether the active view is maximized |
| `viewports[]` | Per visible slot: `viewId`, `type`, `slotIndex`, `name`, `screenRect` (screen coordinates), optional `studyInstanceUID` / `seriesInstanceUID`, `dataID`, orientation, optional `thumbnail` (`contentType`, base64 `data`, `width`, `height` — PNG scaled to max 160px wide) |

Request with `event.context.dataType: "SCENEVIEW"`, `target.actor: "ID"`, optional `target.product.name: "VOLVIEW"`.

## Replacement (CastInterface / 3D Slicer)
...
`cast_py_client`, `VolView/src/io/cast/event-names.ts`, and
